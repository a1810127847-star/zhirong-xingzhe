...
完整解压项目负责人提供的 `智融行者验收一键部署.zip`，双击
`一键部署并打开验收.cmd`。安装器会自动配置 Git、Python、WSL2、Ubuntu 22.04、
ROS2 Humble 和项目依赖，Clone 公共仓库、构建工作空间，并自动打开验收面板与
完整仿真。首次启用 WSL 时可能需要重启一次，登录后会通过 RunOnce 自动续装。

支持的自动安装基线为 64 位 Windows 10 2004（Build 19041）以上，推荐
Windows 11；机器需要管理员权限和可用的 WinGet。具体交付说明见
`给验收人员的部署说明.txt`。
